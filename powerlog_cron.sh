...
if [ "${new_archive}" = true ]; then
    # compress the previous day's log if it exists and is not already compressed
    prev_date=$(date -d "yesterday" +%Y-%m-%d)
    prev_year=$(date -d "yesterday" +%Y)
    prev_month=$(date -d "yesterday" +%m)
    prev_dir="${archive_dir}/${prev_year}/${prev_month}"
    prev_log="${prev_dir}/${prev_date}.log"
    prev_archive="${prev_log}.zst"

    if [ -f "${prev_log}" ] && [ ! -f "${prev_archive}" ]; then
        if zstd --ultra -22 -f -- "${prev_log}"; then
            rm -f -- "${prev_log}"
        fi
    fi
fi
